...
?>
							...
							echo "</tr>";
								...
								<?php	
								$sqlb = "select status from transaksi_penilaian where periode='$periode2'";
...
<?php
	...
	$nikaryawan=array();
	$sql1 = "SELECT nik,(select Nama_Lengkap from master_karyawan a where a.nik=b.nik) as nama, totalnilai AS skor FROM transaksi_penilaian b where periode='$periode' GROUP BY nik ORDER by totalnilai DESC limit 3";
	$result1 = mysqli_query($conn, $sql1);
	if (mysqli_num_rows($result1) > 0) {
	while($row = mysqli_fetch_assoc($result1)) {
		$nik = $row["nik"];
...
$(function () {
	...
	$("#bagianlist").DataTable({
      ...
      "autoWidth": true
								...
								var barChartData0 = {
								  labels  : [<?php echo $_SESSION['skorkar'];?>],
								  datasets: [
									{
									  label               : 'Total Nilai',
									  backgroundColor     : 'rgba(60,141,188,0.9)',
