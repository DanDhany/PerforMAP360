...
<?php
	...
	$nikaryawan=array();
	$skor=array();
	$sql1 = "SELECT nik,(select Nama_Lengkap from master_karyawan a where a.nik=b.nik) as nama, totalnilai AS skor FROM transaksi_penilaian b where periode='$periode' GROUP BY nik ORDER by totalnilai asc";
	$result1 = mysqli_query($conn, $sql1);
	if (mysqli_num_rows($result1) > 0) {
	while($row = mysqli_fetch_assoc($result1)) {
		$nik = $row["nik"];
...
?>
	...
	$b = $row1["periode"];
		...
		$sql1 = "SELECT nik,(select Nama_Lengkap from master_karyawan a where a.nik=b.nik) as nama, MAX(totalnilai) AS skor FROM transaksi_penilaian b where periode ='$b' GROUP BY periode";
		$result1 = mysqli_query($conn, $sql1);
		if (mysqli_num_rows($result1) > 0) {
		while($row = mysqli_fetch_assoc($result1)) {
			$nik = $row["nik"];
		...
		$sql2 = "SELECT nik,(select Nama_Lengkap from master_karyawan a where a.nik=b.nik) as nama, MIN(totalnilai) AS skor FROM transaksi_penilaian b where periode ='$b' GROUP BY periode";
