...
?>
							...
							<div class="col-lg-12">
								<h2 style="text-align:center">Laporan Kinerja Karyawan</h2>
							  </div>
							  <div class="card-body">
							...
							echo "</tr>";
								...
								if (mysqli_num_rows($result1) > 0) {
									while($row = mysqli_fetch_assoc($result1)) {
										...
										$_SESSION['nilaikahir'][$x][$a]['totalkriteria'] = $totalkri[$a][$x] = ($nilai[$a][$x] / $jumsub[$a][$x]) * ($botkri[$a][$x]/100);
									...
									echo "<td><b>Sub Total Nilai</b></td>";
										echo "<td><b>";
										echo $_SESSION['nilaikahir'][$a]['jumbobot'] =  array_sum($totalbot[$a]);
								...
								where kdpenilai='$a' and kdnilai in (SELECT kdnilai from transaksi_penilaian WHERE nik='$nikpilih' and periode='$periode3' and status='1') group by kdkri";
								...
								<table width="100%" class="table table-striped table-bordered table-hover">
								<tr><td style="text-align:center"><b></b></td><td colspan='4' style="text-align:center"><b>Penilai</b></td></tr>
								<tr><td style="text-align:center"><b>Kriteria</b></td>
								<?php
								$sqla = "SELECT * FROM `master_penilai`";
								$resulta = mysqli_query($conn, $sqla);
								if (mysqli_num_rows($resulta) > 0) {
								while($row1 = mysqli_fetch_assoc($resulta)) {
									...
									$b = $row1["rolepenilai"];
									$c = $row1["ketrole"];
									echo "<td style=\"text-align:center\"><b>".$b."</b></td>";
								}
								}
								?>
								</tr>
								...
								<?php
								$sqla = "SELECT * FROM `master_kriteria`";
								$resulta = mysqli_query($conn, $sqla);
								if (mysqli_num_rows($resulta) > 0) {
								while($row1 = mysqli_fetch_assoc($resulta)) {
								$a = $row1["kdkri"];
								$x = $row1["namkri"];
								echo "<tr><td><b>".$x."</b></td>";
										$sqlb = "SELECT * FROM `master_penilai`";
										$resultb = mysqli_query($conn, $sqlb);
										if (mysqli_num_rows($resultb) > 0) {
										while($rowb = mysqli_fetch_assoc($resultb)) {
											$b = $rowb["kdpenilai"];
											if(isset($_SESSION['nilaikahir'][$a][$b]['totalkriteria'])){
											echo "<td><b>".$_SESSION['nilaikahir'][$a][$b]['totalkriteria']."</b></td>";
											} else{
												echo "<td><b>"."</b></td>";
											}
										}
								...
								?>
								<tr><td style="text-align:left">Total Sub Nilai</td>
								<?php
								$sqla = "SELECT * FROM `master_penilai`";
								$resulta = mysqli_query($conn, $sqla);
								if (mysqli_num_rows($resulta) > 0) {
									...
									echo $_SESSION['nilaikahir'][$a]['total'];
									echo "</td>";
									}
								}
								echo "</tr>";
								...
								<tr>
									<td colspan='5'><b>Rating Akhir - <span style="color:<?php echo $color;?>"><?php echo $ketrat;?></span></b></td>
								</tr>
								</table>
								...
								?>
									...
									<tr>
									<td colspan='2'><span style="color:red"><b>Untuk Mencetak Laporan Tekan CTRL+P atau gunakan fungsi Print pada Browser anda</span></b></td>
									</tr>
								<?php
								...
								?>
									...
									<tr>
									<td colspan='2'><b><span style="color:red"><b>Untuk Mencetak Laporan Tekan CTRL+P atau gunakan fungsi Print pada Browser anda</span></b></td>
									</tr>
								<?php
								} else{
								?>
...
?>
								...
								<div class="card card-success">
								  <div class="card-header">
									<h3 class="card-title">Perangkingan Nilai Karyawan Periode <?php echo $periode;?></h3>
								...
								$sql1 = "SELECT a.nik, (select Nama_Lengkap from master_karyawan b where b.nik=a.nik group by nik) as nama, periode, totalnilai, status from transaksi_penilaian a where a.periode='$periode' group by a.nik order by nik";
								$result1 = mysqli_query($conn, $sql1);
								if (mysqli_num_rows($result1) > 0) {
									while($row = mysqli_fetch_assoc($result1)) {
										echo "<tr><form action=\"\" method=\"POST\">"; 
