...
if($_POST['tglselesai'] < $_POST['tglmulai']){
	echo "<script>alert(\"Tanggal Salah!\")</script>";
	echo '<script>window.location.href = "";</script>';
} else if($_POST['tglselesai'] == $_POST['tglmulai']){
	echo "<script>alert(\"Tanggal Mulai dan Selesai Tidak Boleh Sama!\")</script>";
	echo '<script>window.location.href = "";</script>';
}  
else {
...
?>
                                    <form role="form" action="" method="POST">
										...
										<label>Pilih Karyawan</label>
											...
											<?php
											$query = "select nik, nama_lengkap, bagian from master_karyawan WHERE NIKatasan ='$nik' OR bagian = '$bagian'";
											$tampil = mysqli_query($conn, $query);
											$muncul = "var detail1 = new Array();\n";
											while($data1 = mysqli_fetch_array($tampil))
											{
