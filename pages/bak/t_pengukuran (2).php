...
$periode = $_SESSION["periode"] = $_POST['periode'];
	...
	$sqlb = "select count(kdnilai) as cek from transaksi_penilaian where nik='$karyawan' and periode='$periode' and kdpenilai='$penilai'";
	$result= mysqli_query($conn, $sqlb);
	$rowb = mysqli_fetch_assoc($result);
	if($rowb['cek'] != '0'){
		echo "<script>alert(\"Data penilaian periode dengan penilai yang dipilih sudah diinputkan!\")</script>";
