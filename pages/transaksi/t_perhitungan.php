...
if ($row['cek'] == ''){
$sql2 = "update transaksi_penilaian set status='2', totalnilai='$a' where periode='$b' and nik='$c'";
mysqli_query($conn, $sql2);
echo "<script>toastr.success(\"Data penilaian dikirim!\")</script>";
} else{
...
else if(isset($_POST['batalkirim'])){
$a = $_POST['totnilai'];
$b = $_POST['periodepilih'];
$c = $_POST['nikkar'];
$d = $_POST['skorakhir'];
$sql2 = "update transaksi_penilaian set status='', totalnilai='$a' where periode='$b' and nik='$c'";
mysqli_query($conn, $sql2);
echo "<script>toastr.warning(\"Data penilaian dibatalkan!\")</script>";
}
...
?>
							...
							<table width="100%" style="display:none">
                                ...
                                <tbody>
								<?php
							/* $sqlc = "select count(kdpenilai)as jumpenilai from master_penilai where kdpenilai";
							$resulta= mysqli_query($conn, $sqlc);
							$rowjum = mysqli_fetch_assoc($resulta);
							$jumpencek = $rowjum['jumpenilai'];	 */
							
							/* $sqld = "select max(periode) as periodeSB from transaksi_penilaian where periode <  '$periode2'";
							...
							echo "</tr>";
								...
								(SELECT COUNT(DISTINCT kdsub) from detail_nilai a WHERE a.kdkri = b.kdkri and kdkri in 
								...
								(select bobot from master_bobot a where a.kdpenilai=b.kdpenilai and a.kdkri = b.kdkri group by kdkri and kdpenilai) as bobot, 
								(SELECT COUNT(DISTINCT kdsub) from detail_nilai a WHERE a.kdkri = b.kdkri) as jumsub from detail_nilai b 
								...
								<?php
								$sqlp = "select count(kdpenilai) as pen from master_penilai where kdpenilai in (select kdpenilai from master_bobot)";
								$resultp= mysqli_query($conn, $sqlp);
								$rowp = mysqli_fetch_assoc($resultp);
								$hitungPenilai = $rowp['pen'];
								...
								if($rowb['status'] != ''){
									$cekpenilai = $hitungPenilai;
									
								} else {
									$sqlc = "select count(kdpenilai) as status from transaksi_penilaian where periode='$periode2' and nik='$nikpilih'";
									$result= mysqli_query($conn, $sqlc);
									$rowc = mysqli_fetch_assoc($result);
									$cekpenilai = $rowc['status'];
									
								}
								if($cekpenilai == $hitungPenilai){
								?>
									<tr>
									<td colspan='2'><b><label>Data Kinerja Sudah Lengkap, Status Penilaian <span style="color:green">Sudah Selesai</span></label></b></td>
								...
								?>
									<tr>
									<td colspan='2'><b><label>Laporan Sudah <span style="color:orange">Diserahkan</span>. Apakah anda ingin merubah status?</label></b></td>
										<td style="text-align:center"><b>
										<button type="submit" name="batalkirim" class="btn btn-success">Iya</button>	
										</b></td>
									</tr>
									<tr>
								...
								<?php
								} else if($rowb['status'] == '1'){
								?>
									<tr>
									<td colspan='2'><b><label>Laporan Sudah <span style="color:green">Diterima</span></label></b></td>
									</tr>
									<tr>
									<td colspan='2'><span style="color:blue"><b>Untuk Mencetak Laporan Tekan CTRL+P atau gunakan fungsi Print pada Browser anda</span></b></td>
									</tr>
								...
								?>
									<tr>
									<td colspan='2'><b><label>Laporan Sudah <span style="color:red">Ditolak</span></label></b></td>
								...
								?>
									<tr>
									<td colspan='2'><b>Kirim Laporan ?</b></td>
										<td style="text-align:center"><b>
										<button type="submit" name="kirimnilai" class="btn btn-success">Iya</button>	
										</b></td>
									...
									<td colspan='2'><b>Keterangan:
									<span style="color:black"><b>
									Standar Minimal Kinerja tercapai apabila karyawan memiliki total nilai minimal 3 (tiga), jika karyawan mendapat nilai kurang dari 3 (tiga) maka keryawan yang bersangkutan perlu mengikuti pembinaan lebih lanjut.
									</span></b></td>
									</tr>
								<?php
								}
								...
								?>
									<tr>
									<td colspan='2'><b><label>Data Kinerja Belum Lengkap, Status Penilaian <span style="color:red">Belum Selesai</span></label></b></td>
...
?>
								...
								<table width="100%" class="table table-striped table-bordered table-hover" id="bagianlist2">
								...
								$rangkingkar2=1;
								$sql1 = "SELECT a.nik, (select Nama_Lengkap from master_karyawan b where b.nik=a.nik group by nik) as nama, periode, totalnilai, status from transaksi_penilaian a where a.periode='$periode' group by a.nik order by totalnilai desc";
								$result1 = mysqli_query($conn, $sql1);
								if (mysqli_num_rows($result1) > 0) {
									while($row = mysqli_fetch_assoc($result1)) {
										...
										else if($row["status"]=='2'){echo "<b><span style=\"color:blue\">Diserahkan</span></b>";}
										else {
											$nikhitung = $row["nik"];
											$sqlp = "select count(kdpenilai) as pen from master_penilai where kdpenilai in (select distinct kdpenilai from master_bobot)";
											$resultp= mysqli_query($conn, $sqlp);
											$rowp = mysqli_fetch_assoc($resultp);
											$hitungPenilai = $rowp['pen'];
											
											$sqlb = "select count(kdpenilai) as status from transaksi_penilaian where periode='$periode' and nik='$nikhitung'";
											$result= mysqli_query($conn, $sqlb);
											$rowb = mysqli_fetch_assoc($result);
											if ($rowb['status']==$hitungPenilai){
												echo "<b><span style=\"color:blue\">Menunggu Dikirim(".$rowb['status']." dari ".$hitungPenilai." Telah Masuk)</span></b>";
											} else{
												echo "<b><span style=\"color:purple\">Belum Selesai (".$rowb['status']." dari ".$hitungPenilai." Telah Masuk)</span></b>";
											}
...
?>
                                ...
                                <thead>
                                    <tr>
										...
										<th>Penilaian Menunggu Diserahkan</th>
										<th>Proses</th>
										</tr>
                                </thead>
								<tbody>
								...
								$sql1 = "SELECT periode from transaksi_penilaian a  group by a.periode order by periode desc";
								$result1 = mysqli_query($conn, $sql1);
								if (mysqli_num_rows($result1) > 0) {
									while($row = mysqli_fetch_assoc($result1)) {
									$period = $row["periode"];
									...
									/* $sqlp = "select count(kdpenilai) as pen from master_penilai";
									$resultp= mysqli_query($conn, $sqlp);
									$rowp = mysqli_fetch_assoc($resultp);
									$hitungPenilai = $rowp['pen'];
									
									$sqlc = "SELECT count(nik) as sudahnilai from master_karyawan a WHERE nik in (SELECT nik from transaksi_penilaian where periode='$period' HAVING COUNT(kdpenilai) = $hitungPenilai)";
									$resultc = mysqli_query($conn, $sqlc);
									$rowc = mysqli_fetch_assoc($resultc); 
									$sudahnilai = $rowc['sudahnilai']; */
										...
										echo "<b>".$niksudah."</b>";
										echo "</td>";
											$sqlp = "select count(kdpenilai) as pen FROM `master_penilai` a where a.kdpenilai in (select distinct kdpenilai from master_bobot)";
											$resultp= mysqli_query($conn, $sqlp);
											$rowp = mysqli_fetch_assoc($resultp);
											$hitungPenilai = $rowp['pen'];
											
											$sqlb = "SELECT COUNT(*) as siap FROM (SELECT COUNT(DISTINCT nik) FROM transaksi_penilaian where periode='$period' and status='' 
											GROUP by nik HAVING COUNT(nik) = '$hitungPenilai') as hitung";
											
											//$sqlb = "SELECT COUNT(nik) as siap FROM transaksi_penilaian where periode='$period' and status=''";
											
											$result= mysqli_query($conn, $sqlb);
											$rowb = mysqli_fetch_assoc($result);
											$siap = $rowb['siap'];
											$totsiap = $siap/$hitungPenilai;
											
										echo "<td>";
										//echo "<b>".round($totsiap,0)."</b>";
										echo "<b>".$siap."</b>";
										echo "</td>";
